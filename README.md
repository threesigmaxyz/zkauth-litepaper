...
### Protocol Actors
#### User
The user (`U`) is an off-chain identity that is tied to a web2 identity (eg. Google, Microsoft, etc). The user can operate any zkAuth account that is linked to their web2 identity. The user is responsible for creating and managing their zkAuth account. This includes configuring the account rules, linking the account to their web2 identity, and managing the account recovery mechanism.

#### OpenID Provider
An OpenID Provider (`OP`) is a dedicated, purpose-built service that is responsible for authenticating users and issuing signed JWTs containing [ID tokens](https://openid.net/specs/openid-connect-core-1_0.html#IDToken). These tokens are used to verify the identity of the user and authorize the execution of zkAuth transactions. The zkAuth protocol supports any OIDC compliant provider, such as Google, Microsoft, etc.

#### zkAuth Account
A zkAuth account (`A`) is a smart contract account that is managed by the zkAuth smart contract system. 

The zkAuth account is responsible for managing the lifecycle of a zkAuth session and executing zkAuth transactions. The zkAuth account is configured by the user and can be used to execute transactions on the zkSync L2.
-->

<!--
### Glossary

The following section provides an introduction to the glossary that specifies the notation used to describe the protocol flows.
...
| --- | --- |
| `ZKS` | The zkSync L2. |
| `SC` | The zkAuth smart contracts deployed on ZKS |
| `OP` | An OpenID provider (eg. Google, Microsoft, etc). |
| `USER` | A system user that submits transactions to ZKS. |

- **Keys:**

| Term | Description |
| --- | --- |
| `PK` | An ECDSA public key. |
| `SK` | An ECDSA private (secret) key. |
| `<PK, SK>` | An ECDSA key pair (private and public keys). |

- **Transactions:**

| Term | Description |
| --- | --- |
| `T` | The original zkSync transaction. |
| `H(T)` | The `keccak256` hash of the transaction T. |
| `σ(T)` | An ECDSA signature over transaction T. |
...
| `JWT.<claim>` | A specific claim of a decoded JWT (eg. JWT.sub for the subject claim). |

- **Session**:

| Term | Description |
| --- | --- |
| `S` | A zkAuth session object, composed of a `(<PK, SK>, JWT)` where `JWT.nonce = PK` and `JWT.exp` is not expired. |
-->
